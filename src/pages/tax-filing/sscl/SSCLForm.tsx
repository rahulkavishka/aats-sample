...
import type { SSCLStatus } from './SSCLPage';

export interface SSCLFormData {
    clientId: string;
    clientName: string;
    period: string;
    periodUnit: string;
...
interface SSCLFormProps {
    onSubmit: (data: SSCLFormData) => void;
}

const SSCLForm: React.FC<SSCLFormProps> = ({ onSubmit }) => {
    const { register, handleSubmit, setValue, watch, reset, formState: { errors } } = useForm<SSCLFormData>({
        defaultValues: {
            periodUnit: 'Months',
            status: 'Pending',
        },
    ...
    const onFormSubmit = (data: SSCLFormData) => {
        onSubmit(data);
        reset();
    };
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
                        ...
                        <div className="lg:col-span-4 space-y-4">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="ssclNo">SSCL No.</Label>
                                    <Input
                                        id="ssclNo"
                                        placeholder="e.g. SSCL-555666"
                                        {...register('ssclNo', { required: true })}
                                    />
                                    {errors.ssclNo && <span className="text-xs text-red-500">Required</span>}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-2 space-y-4">
                            ...
                            <RadioGroup
                                ...
                                onValueChange={(val: SSCLStatus) => setValue('status', val)}
