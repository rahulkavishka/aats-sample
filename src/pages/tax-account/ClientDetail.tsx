...
const processes = ["Bookkeep", "Tax Amount", "Finalize", "Tax Paid", "Submit"]

export default function TaxAccountClientDetail() {
    const { id } = useParams()
    ...
    const handleStepClick = (step: string) => {
        ...
        if (stepIndex === currentIndex + 1) {
            // Special validation for Submit
            if (step === "Submit" && clientData.sourceDocuments.length === 0) {
                setAlertMessage("At least one source document must be added before the final Submit stage.")
                setAlertOpen(true)
                return
            }
        ...
        <div className="flex flex-col h-full space-y-6 p-4 md:p-6 animate-in slide-in-from-right-4 duration-500 pb-20">
                    ...
                    {processes.map((step, idx) => {
                        ...
                        return (
                            <li key={step} className="shrink basis-0 flex-1 group cursor-pointer" onClick={() => handleStepClick(step)}>
                                ...
                                <div className="mt-3 text-center transition-transform group-hover:scale-105 duration-200">
                                    <span className={cn("text-xs md:text-sm font-medium transition-colors", status === "upcoming" ? "text-muted-foreground" : "text-foreground")}>{step}</span>
                                </div>
                            </li>
                        )
                    })}
            ...
            <div className="pt-8 pb-6 border-t flex flex-col md:flex-row justify-between items-center gap-4 mt-auto">
                ...
                <div className="flex gap-2 order-1 md:order-2">
                    <Button variant="outline" size="sm" onClick={() => navigate("/tax-account/new")}><Edit className="mr-2 h-4 w-4" /> Edit</Button>
            ...
            <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Delete Record</DialogTitle><DialogDescription>Are you sure you want to delete this tax account record?</DialogDescription></DialogHeader>
                    <DialogFooter><Button variant="outline" onClick={() => setDeleteDialogOpen(false)}>Cancel</Button><Button variant="destructive" onClick={() => navigate("/tax-account")}>Delete</Button></DialogFooter>
                </DialogContent>
            </Dialog>
